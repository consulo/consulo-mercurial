...
public class HgReferenceValidator implements InputValidatorEx
{
	...
	private static final Pattern ILLEGAL = Pattern.compile(".*:.*|" + // colon character is not allowed
	...
	boolean hasConflictsWithAnotherBranch(@Nullable String name)
	{
		...
		myErrorText = currentBranch.equals(name) || branches.contains(name) ? String.format("A branch with the \'%s\' name already exists",
				name) : null;
		return myErrorText != null;
	}
