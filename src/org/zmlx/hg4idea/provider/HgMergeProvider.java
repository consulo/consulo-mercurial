				...
				if(wasFileCheckedIn(repo, file))
				{
					...
					if(result != null)
					{
						...
						if(parts.size() < 2)
						{
							...
							new HgCommandResultNotifier(myProject).notifyError(null, HgVcsMessages.message("hg4idea.error.debugancestor.command" +
									".execution"), HgVcsMessages.message("hg4idea.error.debugancestor.command.description"));
						}
						else
						{
