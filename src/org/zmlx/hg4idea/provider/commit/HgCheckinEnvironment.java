...
public class HgCheckinEnvironment implements CheckinEnvironment
{
			...
			Set<String> feedback)
	{
		...
		for(Map.Entry<HgRepository, Set<HgFile>> entry : repositoriesMap.entrySet())
		{
			...
			HgCommitCommand command = new HgCommitCommand(myProject, repo, preparedComment, myNextCommitAmend, myCloseBranch);
	...
	private Set<HgFile> getChangedFilesNotInCommit(VirtualFile repo, Set<HgFile> selectedFiles)
	{
		...
		HgStatusCommand statusCommand = new HgStatusCommand.Builder(true).unknown(false).ignored(false).baseRevision(parents.get(0)).build
				(myProject);
	...
	private boolean mayCommitEverything(final String filesNotIncludedString)
	{
		...
		Runnable runnable = new Runnable()
		{
			public void run()
			{
				choice[0] = Messages.showOkCancelDialog(myProject, HgVcsMessages.message("hg4idea.commit.partial.merge.message",
						filesNotIncludedString), HgVcsMessages.message("hg4idea.commit.partial.merge.title"), null);
			}
		};
	...
	private void addFile(Map<HgRepository, Set<HgFile>> result, ContentRevision contentRevision)
	{
		...
		HgRepository repo = HgUtil.getRepositoryForFile(myProject, contentRevision instanceof HgCurrentBinaryContentRevision ? (
				(HgCurrentBinaryContentRevision) contentRevision).getRepositoryRoot() : ChangesUtil.findValidParentAccurately(filePath));
		if(repo == null)
		{
			return;
		}
	...
	private class HgCommitAdditionalComponent extends DvcsCommitAdditionalComponent
	{
		...
		public HgCommitAdditionalComponent(@NotNull Project project, @NotNull CheckinProjectPanel panel)
		{
			...
			myCommitSubrepos.setToolTipText(XmlStringUtil.wrapInHtml("Commit all subrepos for selected repositories.<br>" + " <code>hg ci " +
					"<i><b>files</b></i> -S <i><b>subrepos</b></i></code>"));
			myCommitSubrepos.setMnemonic('s');
			myPanel.add(myCommitSubrepos, c);
			Collection<HgRepository> repos = HgActionUtil.collectRepositoriesFromFiles(HgUtil.getRepositoryManager(myProject),
					myCheckinPanel.getRoots());
