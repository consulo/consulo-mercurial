...
public class HgWorkingCopyRevisionsCommand
{
	...
	public Couple<HgRevisionNumber> parents(@NotNull VirtualFile repo, @Nullable FilePath file, @Nullable HgRevisionNumber revision)
	{
		...
		{
			case 1:
				return Couple.of(revisions.get(0), null);
			case 2:
				return Couple.of(revisions.get(0), revisions.get(1));
			default:
		...
		if(lines != null && !lines.isEmpty())
		{
			...
			if(parts.size() >= 2)
			{
				if(changesets.indexOf('+') != changesets.lastIndexOf('+'))
				{
					...
					return Couple.of(HgRevisionNumber.getInstance(revs.get(0) + "+", chsets.get(0) + "+"), HgRevisionNumber.getInstance(revs.get(1)
							+ "+", chsets.get(1) + "+"));
				}
				else
				{
	...
	public
	@NotNull
	List<HgRevisionNumber> getRevisions(@NotNull VirtualFile repo,
