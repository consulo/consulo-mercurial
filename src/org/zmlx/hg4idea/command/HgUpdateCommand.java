...
public class HgUpdateCommand
{
	...
	public HgCommandResult execute()
	{
		...
		try
		{
			...
			{
				final String message = "<html>Your uncommitted changes couldn't be merged into the requested changeset.<br>" + "Would you like to " +
						"perform force update and discard them?";
	...
	public static int showDiscardChangesConfirmation(@NotNull final Project project, @NotNull final String confirmationMessage)
	{
		...
		UIUtil.invokeAndWaitIfNeeded(new Runnable()
		{
			...
			public void run()
			{
				exitCode.set(Messages.showOkCancelDialog(project, confirmationMessage, "Uncommitted Changes Problem", "&Discard Changes", "&Cancel",
			...
			final boolean clean)
	{
		...
		{
			new VcsNotifier(project).notifyImportantWarning("Unresolved conflicts.", HgVcsMessages.message("hg4idea.update.warning.merge.conflicts",
					repository.getPath()));
		}
