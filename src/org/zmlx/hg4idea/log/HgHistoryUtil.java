...
public class HgHistoryUtil
{
			...
			@NotNull List<String> parameters) throws VcsException
	{
		...
		HgBaseLogParser<VcsCommitMetadata> baseParser = new HgBaseLogParser<VcsCommitMetadata>()
		{
					...
					@NotNull List<String> attributes)
			{
				...
				return factory.createCommitMetadata(factory.createHash(changeset), parentsHash, revisionDate.getTime(), root, subject, author,
						email, message, author, email, revisionDate.getTime());
			}
		};
			...
			boolean silent)
	{
		...
		List<HgFileRevision> hgRevisions = getCommitRecords(project, result, new HgFileRevisionLogParser(project, getOriginalHgFile(project, root),
				version), silent);
		...
		for(HgFileRevision revision : hgRevisions)
		{
			...
			vcsFullCommitDetailsList.add(factory.createFullDetails(factory.createHash(vcsRevisionNumber.getChangeset()), parentsHash,
					revision.getRevisionDate().getTime(), root, vcsRevisionNumber.getSubject(), vcsRevisionNumber.getAuthor(),
					vcsRevisionNumber.getEmail(), vcsRevisionNumber.getCommitMessage(), vcsRevisionNumber.getAuthor(), vcsRevisionNumber.getEmail(),
					revision.getRevisionDate().getTime(), new ThrowableComputable<Collection<Change>, Exception>()
			{
				@Override
			...
			@NotNull List<String> hashes) throws VcsException
	{
		...
		return getCommitRecords(project, result, new HgBaseLogParser<VcsShortCommitDetails>()
		{
					...
					@NotNull List<String> attributes)
			{
				...
				return factory.createShortDetails(factory.createHash(changeset), parentsHash, revisionDate.getTime(), root, subject, author, email,
	...
	@NotNull
	public static List<TimedVcsCommit> readAllHashes(@NotNull Project project,
			@NotNull VirtualFile root,
			@NotNull final Consumer<VcsUser> userRegistry,
			@NotNull List<String> params) throws VcsException
	{
	...
	@NotNull
	public static Change createChange(@NotNull Project project,
			@NotNull VirtualFile root,
			...
			FileStatus aStatus)
	{
		...
		HgContentRevision beforeRevision = fileBefore == null ? null : new HgContentRevision(project, new HgFile(root, new File(root.getPath(),
				fileBefore)), revisionBefore);
		HgContentRevision afterRevision = fileAfter == null ? null : new HgContentRevision(project, new HgFile(root, new File(root.getPath(),
				fileAfter)), revisionAfter);
	...
	@NotNull
	public static Collection<String> getDescendingHeadsOfBranches(@NotNull Project project,
			@NotNull VirtualFile root,
